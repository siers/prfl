...
class ToneLib {
  ...
  namesSemiMap = { c: 0, d: 2, e: 3, f: 5, g: 7, a: 9, b: 11 }
  ...
  noteWithRender(note) {
    return { ...note, render: this.renderNote(note), octave: Math.floor((note.semi - 4) / 12) + 1 }
  }
  ...
  parseNote(note) {
    ...
    const semiBaseC = parseInt((match.groups.octave || 4) - 1, 10) * 12 + this.c1
    const semi = semiBaseC + this.namesSemiMap[match.groups.note]
  ...
  rebase({ semi, name, alter, render }, baseSemi) {
    return this.noteWithRender({
      semi: baseSemi + ((12000 + semi - this.c1) % 12),
      name,
      alter,
    })
  }
  ...
  major() {
    return 'cdefgab'.split('').map(n => this.parseNote(n))
  }
  ...
  majorKey(note) {
    return this.keysMajor().find(k => this.renderNote(k[0], false) == this.renderNote(note, false))
  }
  ...
  allNotes() {
    const c4 = this.parseNote('C4')
    return this.keysMajor().flat().map(n => this.rebase(n, c4.semi))
  }
...
if (typeof window !== 'undefined') window.ToneLib = new ToneLib
